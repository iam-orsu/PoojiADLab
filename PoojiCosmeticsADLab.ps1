...
# DomainController (Keerthipati-DC) and Both Workstation (Cyberprincess & Firediva)  
#  -- Autoconfigured IP Addresses --
#  DC will always have ip x.x.x.250
#  Cyberprincess will always have ip x.x.x.220 
#  Firediva will always have ip x.x.x.221
...
function fix_setspn {
  ...
  #setspn -D SQLService/POOJICOSMETICS.local KEERTHIPATI-DC > $null
  #setspn -D SQLService/Poojicosmetics.local POOJICOSMETICS\SQLService > $null
  #setspn -D KEERTHIPATI-DC/SQLService.POOJICOSMETICS.local:60111 POOJICOSMETICS\SQLService > $null
  #setspn -D POOJICOSMETICS/SQLService.Poojicosmetics.local:60111 POOJICOSMETICS\SQLService > $null
  #setspn -D DomainController/SQLService.POOJICOSMETICS.Local:60111 POOJICOSMETICS\SQLService > $null
...
#--- new code 
  ...
  # setspn -A KEERTHIPATI-DC/SQLService.POOJICOSMETICS.local:60111 POOJICOSMETICS\SQLService > $null
  # setspn -A SQLService/POOJICOSMETICS.local  POOJICOSMETICS\SQLService > $null
  # setspn -A DomainController/SQLService.POOJICOSMETICS.local:60111 POOJICOSMETICS\SQLService > $null
 ...
 setspn -A DomainController/SQLService.$FullDomainName`:60111 $ShortDomainName\SQLService > $null
  ...
  write-host("`n  [++] Checking Local Keerthipati-DC SPN")
  #setspn -L KEERTHIPATI-DC
  # -- new code 
  setspn -L $machine 
  write-host("`n  [++] Checking POOJICOSMETICS\SQLService SPN")
  #setspn -L POOJICOSMETICS\SQLService
  # -- new code 
...
function build_lab {
  ...
  # install adds 
  write-host("`n  [++] Installing ADDS Domain : Poojicosmetics.local ")
  Install-ADDSDomain -SkipPreChecks -ParentDomainName POOJICOSMETICS -NewDomainName local -NewDomainNetbiosName POOJICOSMETICS `
  ...
  # create adds forest poojicosmetics.local
  write-host("`n  [++] Deploying Active Directory Domain Forest in POOJICOSMETICS.local")
  Install-ADDSForest -SkipPreChecks -CreateDnsDelegation:$false -DatabasePath "C:\Windows\NTDS" `
  -DomainMode "WinThreshold" -DomainName "POOJICOSMETICS.local" -DomainNetbiosName "POOJICOSMETICS" `
...
function create_labcontent {
  ...
  # create user manasa
  New-ADUser -Name "Manasa C" -GivenName "Manasa" -Surname "C" -SamAccountName "manasa" `
  -UserPrincipalName "manasa@$Global:Domain -Path DC=poojicosmetics,DC=local" `
  ...
  Write-Host "`n  [++] User: Manasa C added, Logon: manasa Password: Password2"
  Write-Host "        Adding Manasa C to Poojicosmetics.local Groups: Domain Users"

  # create user pooji
  New-ADUser -Name "Poojitha Keerthipati" -GivenName "Poojitha" -Surname "Keerthipati" -SamAccountName "pooji" `
  -UserPrincipalName "pooji@$Global:Domain -Path DC=poojicosmetics,DC=local" `
  -AccountPassword (ConvertTo-SecureString "Password1" -AsPlainText -Force) `
  -PasswordNeverExpires $true -PassThru | Enable-ADAccount  | Out-Null

  # if the rps_s_access_denited is fixed by the reg key, pooji no longer needs to be a domain admin
  Add-ADGroupMember -Identity "Domain Admins" -Members pooji  | Out-Null
  Write-Host "`n  [++] User: Poojitha Keerthipati added, Logon: pooji Password: Password1"
  Write-Host "        Adding Poojitha Keerthipati to Poojicosmetics.local Groups: Domain Users, Domain Admins"

  # create user orsu 
  New-ADUser -Name "Vamsi Krishna" -GivenName "Vamsi" -Surname "Krishna" -SamAccountName "orsu" `
  -UserPrincipalName "orsu@$Global:Domain -Path DC=poojicosmetics,DC=local" `
  -AccountPassword (ConvertTo-SecureString "Password2019!@#" -AsPlainText -Force) `
  -PasswordNeverExpires $true -PassThru | Enable-ADAccount | Out-Null

  Add-ADGroupMember -Identity "Administrators" -Members orsu
  Add-ADGroupMember -Identity "Domain Admins" -Members orsu
  Write-Host "`n  [++] User: Vamsi Krishna added, Logon: orsu Password: Password2019!@#"
  Write-Host "        Adding Vamsi Krishna to Poojicosmetics.local Groups: Administrators, Domain Admins"

  # create user sqlservice 
  New-ADUser -Name "SQL Service" -GivenName "SQL" -Surname "Service" -SamAccountName "sqlservice" `
  -UserPrincipalName "sqlservice@$Global:Domain -Path DC=poojicosmetics,DC=local" `
  ...
  Write-Host "        Adding SQLService to Poojicosmetics.local Groups: Administrators, Domain Admins, Enterprise Admins, Group Policy Creator Owners, Schema Admins"
  ...
  # create ou=groups, move all existing groups into ou=groups,dc=poojicosmetics,dc=local
  New-ADOrganizationalUnit -Name "Groups" -Path "DC=POOJICOSMETICS,DC=LOCAL" -Description "Groups" | Out-Null
  get-adgroup "Schema Admins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Allowed RODC Password Replication Group" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Cert Publishers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Cloneable Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Denied RODC Password Replication Group" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "DnsAdmins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "DnsUpdateProxy" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Domain Computers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Domain Guests" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Domain Users" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Domain Admins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Enterprise Admins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Enterprise Key Admins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Enterprise Read-only Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Group Policy Creator Owners" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Key Admins" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Protected Users" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "RAS and IAS Servers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  get-adgroup "Read-only Domain Controllers" | move-adobject -targetpath "OU=Groups,DC=POOJICOSMETICS,DC=LOCAL" | Out-Null
  }
  # ---- end create_labcontent function


  # ---- begin create_poojicosmetics_gpo
function create_poojicosmetics_gpo {
  ...
  Get-GPO -Name "Disable Defender" | New-GPLink -target "DC=POOJICOSMETICS,DC=local" -LinkEnabled Yes -Enforced Yes
...
# ---- begin set_cyberprincess_staticip function  
function set_cyberprincess_staticip { 
  # get the ip address
  $IPAddress=Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex $(Get-NetConnectionProfile | Select-Object -ExpandProperty InterfaceIndex) | Select-Object -ExpandProperty IPAddress
  ...
  # ---- end set_cyberprincess_staticip function  

# ---- begin set_firediva_staticip function  
function set_firediva_staticip { 
  # get the ip address
  $IPAddress=Get-NetIPAddress -AddressFamily IPv4 -InterfaceIndex $(Get-NetConnectionProfile | Select-Object -ExpandProperty InterfaceIndex) | Select-Object -ExpandProperty IPAddress
  ...
  # ---- end set_firediva_staticip function
...
function fix_workstationdns {
  $DCDNS=(Test-Connection -comp KEERTHIPATI-DC -Count 1).ipv4address.ipaddressToString
  
  write-host("`n  [++] Found KEERTHIPATI-DC At $DCDNS")
  $adapter=Get-CimInstance -Class Win32_NetworkAdapter -Property NetConnectionID,NetConnectionStatus | Where-Object { $_.NetConnectionStatus -eq 2 } | Select-Object -Property NetConnectionID -ExpandProperty NetConnectionID
...
function server_build {
  ...
  if($currentname -ne "KEERTHIPATI-DC") {
      write-host("`n  Computer Name is Incorrect Setting KEERTHIPATI-DC")
      write-host("`n  - Script Run 1 of 3 - Setting the computer name to KEERTHIPATI-DC and rebooting")
      ...
      set_dcstaticip
      Rename-Computer -NewName "KEERTHIPATI-DC" -Restart
      }
      elseif ($domain -ne "POOJICOSMETICS.LOCAL") {
      ...
      elseif ($domain -eq "POOJICOSMETICS.LOCAL" -And $machine -eq "KEERTHIPATI-DC") {
        write-host("`n Computer name and Domain are correct : Executing CreateContent Function ")
        create_labcontent
        create_poojicosmetics_gpo
        get_sharphound
        fix_dcdns 
...
function workstations_common { 
  ...
  # automatically join domain using orsu
  write-host("`n Joining machine to domain Poojicosmetics.local")
  # add-computer -domainname "POOJICOSMETICS.LOCAL" -username administrator -restart | Out-Null
  $domain = "POOJICOSMETICS"
  $password = "Password2019!@#" | ConvertTo-SecureString -asPlainText -Force
  $username = "$domain\orsu" 
  $credential = New-Object System.Management.Automation.PSCredential($username,$password)
  Add-Computer -DomainName $domain -Credential $credential  | Out-Null 
  }
  # ---- end workstations_common function      

# ---- begin workstation_cyberprincess function 
function workstation_cyberprincess { 
  write-host("`n`n   Computer Name : $machine")
  write-host("     Domain Name : $domain")
  write-host("      OS Version : $osversion")

  if ($machine -ne "CYBERPRINCESS") { 
    write-host ("`n Setting the name of this machine to CYBERPRINCESS and rebooting automatically...")
    write-host (" Run this script 1 more time and select 'P' in the menu to join the domain")
    Read-Host -Prompt "`n Press ENTER to continue..."
    # set_mppref
    set_cyberprincess_staticip 
    Rename-Computer -NewName "CYBERPRINCESS" -Restart
    }
    elseif ($machine -eq "CYBERPRINCESS") {
      workstations_common
      ...
      Read-Host -Prompt "`n All done! $machine is all setup! `n Press Enter to reboot and Login as POOJICOSMETICS\pooji and Password1 "
      restart-computer 
    }
    else { write-host("Nothing to do here") }
    } 
    # ---- end workstation_cyberprincess function 
    
# ---- begin workstation_firediva function
function workstation_firediva { 
  write-host("`n`n   Computer Name : $machine")
  write-host("     Domain Name : $domain")
  write-host("      OS Version : $osversion")
  
  if ($machine -ne "FIREDIVA") {
    write-host ("`n Setting the name of this machine to FIREDIVA and rebooting automatically...")
    write-host (" Run this script 1 more time and select 'S' in the menu to join the domain")
    Read-Host -Prompt "`n Press ENTER to continue..."
    set_mppref
    set_firediva_staticip
    Rename-Computer -NewName "FIREDIVA" -Restart
    }
    elseif ($machine -eq "FIREDIVA") {
      workstations_common 
      Get-LocalUser -Name "Administrator" | Enable-LocalUser
      $UserAccount = Get-LocalUser -Name "Administrator"
      $UserAccountPassword = "Password1" | ConvertTo-SecureString -asPlainText -Force
      $UserAccount | Set-LocalUser -Password $UserAccountPassword
      #add pooji as a local administrator on the firediva machine 
      Add-LocalGroupMember -Group Administrators -Member Pooji -Verbose
      Read-Host -Prompt "`n All done! $machine is all setup! `n Press Enter to reboot and Login as POOJICOSMETICS\manasa and Password2 "
      restart-computer 
      }
    else { write-host("Nothing to do here") }
    } 
    # ---- end workstation_firediva function

# ---- begin menu function
function menu {
  do {
    Write-Host "`n`n`t Pooji Cosmetics AD-Lab Build Menu - Select an option`n"
    Write-Host "`tPress 'D' to setup Keerthipati-DC Domain Controller"
    Write-host "`t(must be run 3 times)`n"
    Write-Host "`tPress 'P' to setup Cyberprincess Workstation and join the domain Poojicosmetics.local"
    Write-host "`t(must be run 2 times)`n"
    Write-Host "`tPress 'S' to setup Firediva Workstation and join the domain Poojicosmetics.local" 
    Write-host "`t(must be run 2 times)`n"
  ...
  switch ($choice) {
    'D'{  Write-Host "`n Running... Keerthipati-DC domain controller"
          nukedefender 
          server_build }
    'P'{  Write-Host "`n Running... Cyberprincess Workstation"
          nukedefender 
          workstation_cyberprincess }
    'S'{  Write-Host "`n Running... Firediva Workstation"
          nukedefender 
          workstation_firediva }
